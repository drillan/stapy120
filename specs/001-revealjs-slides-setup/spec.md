...
### User Story 4 - スライド番号の表示とURLによる直接アクセス (Priority: P2)

発表者や聴衆がスライドの現在位置を把握でき、特定のスライドにURLで直接アクセスできる。

**Why this priority**: スライド共有時に特定ページへのリンクを提供でき、プレゼンテーションの利便性に直結する

**Independent Test**: スライドを表示して右下にページ番号が表示されること、URLハッシュでスライドに直接アクセスできることを確認できる

**Acceptance Scenarios**:

1. **Given** スライドをブラウザで表示した状態, **When** 任意のスライドに遷移する, **Then** 右下に「現在のページ番号/総ページ数」形式でスライド番号が表示される
2. **Given** スライドをブラウザで表示した状態, **When** スライドを遷移する, **Then** URLのハッシュ部分がスライド位置に応じて更新される
3. **Given** スライドのURLにハッシュ（例: `#/1/2`）が含まれている, **When** そのURLにアクセスする, **Then** 該当するスライドが直接表示される
...
- **FR-009**: スライド表示時に「現在のページ番号/総ページ数」形式でスライド番号が表示されなければならない
- **FR-010**: URLハッシュによるスライドへの直接アクセスが可能でなければならない（`revealjs_script_conf`の`hash`設定）
...
- **SC-005**: スライド表示時に右下にページ番号が表示され、URLハッシュで任意のスライドに直接アクセスできる
