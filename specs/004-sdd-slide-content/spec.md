...
- Q: 見出しレベルの仕様は？ → A: H1がタイトル、H2がセクション区切り（6つ）、H3が個別スライド
...
発表者が `docs/index.md` を開くと、6セクション構成のスライドが完成しており、`uv run make -C docs revealjs` でビルドして Reveal.js スライドとして表示できる。各スライドの文言は natural-text ルールに準拠し、25分の発表に適した分量である。
...
1. **Given** `docs/index.md` が存在する状態, **When** ビルドを実行, **Then** H1タイトル + H2セクション6つ + H3個別スライドが表示される
...
聴衆が「実践例」セクションを見ると、hachimokuプロジェクトを通じてSDDの具体的な成果物（仕様書）が実物として提示され、「自分もやってみたい」と思える状態になる。

**Why this priority**: Constitution原則III「実践による実証」の中核。理論だけでは行動を促せない。

**Independent Test**: hachimokuの仕様書抜粋がスライドに正しく表示されることを目視確認で検証可能。

**Acceptance Scenarios**:

1. **Given** 実践例セクションを表示した状態, **When** 内容を確認, **Then** hachimoku の仕様書から具体的な抜粋が表示される
2. **Given** 参考リンクスライドを表示した状態, **When** リンクを確認, **Then** 聴衆が即座に始められるツールのURLが掲載されている（Constitution原則V）

---

### User Story 4 - Spec Kit実践Tips (Priority: P2)

聴衆が「Spec KitのTips」セクションを見ると、仕様分割・仕様書の読み解き方・Tasks to Issuesの活用法など、実践で役立つ具体的なTipsが提示される。

**Why this priority**: 実践的なTipsは入門者がSpec Kitを使い始める際のハードルを下げる。

**Independent Test**: Tipsセクションの3スライドがブラウザで正しく表示され、各Tipsの内容が具体的であることを目視確認で検証可能。

**Acceptance Scenarios**:

1. **Given** 仕様の分割スライドを表示した状態, **When** 内容を確認, **Then** 仕様分割のTipsと依存関係ダイアグラムが表示される
2. **Given** 仕様書の文量スライドを表示した状態, **When** 内容を確認, **Then** `/speckit.specify` 直後にAIに質問するTipsが表示される
3. **Given** Tasks to Issuesスライドを表示した状態, **When** 内容を確認, **Then** 粒度調整とドキュメント参照のTipsが表示される
...
- **FR-002**: H2 セクションは以下の6つとすること: 「仕様駆動開発(SDD)とは」「SDDのツール」「Spec Kit入門」「実践例」「Spec KitのTips」「まとめ」
...
- **FR-005**: 「仕様駆動開発(SDD)とは」（5〜7スライド程度）— バイブコーディングの課題を提示し、SDDの定義・利点を信頼できる一次情報に基づいて説明すること。従来の開発との対比や、入門者向けのたとえ話を含めること。SDDの欠点・注意点（初期コスト、コンテキスト消費、仕様の品質）を利点の直後に公平に提示すること。用語・ツールの初出時には公式サイトやリポジトリ等のハイパーリンクを明記すること
- **FR-006**: 「SDDのツール」（1〜2スライド程度）— Spec Kit（テンプレート型・品質検証）・Kiro（IDE型・Hooks自動化・マルチモーダル）・cc-sdd（CLI型・Kiro互換・多言語）など主要なSDDツールの固有の特色を紹介すること。複数エージェント対応などの共通特徴ではなく、各ツールの差別化ポイントを記載すること
- **FR-007**: 「Spec Kit入門」（8〜12スライド程度）— Spec Kitの概要と対応エージェントを紹介し、ワークフローの各ステップ（Constitution, Specify, Clarify, Plan, Tasks, Analyze, Checklist, Implement, Tasks to Issues）をそれぞれ説明すること。各ステップには対応する `specify` コマンドを示すこと
- **FR-008**: 「実践例」（2〜4スライド程度）— hachimokuプロジェクトを紹介し、仕様書の実物を `literalinclude` で抜粋表示すること
- **FR-008a**: 「Spec KitのTips」（2〜3スライド程度）— 仕様分割のTips（依存関係ダイアグラム付き）、仕様書の読み解き方（`/speckit.specify` 直後のAI質問）、Tasks to Issuesの粒度調整と関連ドキュメント参照のTipsを含むこと
- **FR-009**: 「まとめ」（2〜3スライド程度）— 発表の要点を要約すること。聴衆が始められるよう参考リンクを掲載すること
